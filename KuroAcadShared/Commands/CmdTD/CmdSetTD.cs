    ...
    class CmdSetTD
    {
        ...
        public void KuroSetTD()
        {
            ...
            using (Transaction acTrans = acDb.TransactionManager.StartTransaction())
            {
                ...
                int rowNum = acTable.Rows.Count(); //Get number of rows in table
                int rowIdx = 5; //Start from row 3
                for (int i = 0; i < rowNum - 5; i++)
                {
                    if (acTable.Cells[rowIdx, XcolumnIndex].Value == null || acTable.Cells[rowIdx, YcolumnIndex].Value == null)
                    {
                        continue;
                    }

                    pts.Add(new Point2d(Convert.ToDouble(acTable.Cells[rowIdx, XcolumnIndex].Value), Convert.ToDouble(acTable.Cells[rowIdx, YcolumnIndex].Value)));
                    rowIdx++;
                }
